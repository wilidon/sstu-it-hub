...
""")
    ...
    @Query("SELECT p FROM Project p WHERE EXISTS (SELECT 1 FROM UserRoleForProject u WHERE u.id = :userId AND u.role IN (SELECT ac.role FROM ActualRoleForProject ac WHERE ac.project.id = p.id))")
